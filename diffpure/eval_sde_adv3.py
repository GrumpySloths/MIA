...
class SDE_Adv_Model(nn.Module):
    ...
    def forward(self, x, x_start, noise):
        ...
        #x_re = self.runner.image_editing_sample_o((x - 0.5) * 2, x_start, e = noise, bs_id=counter, tag=self.tag)
        x_re = self.runner.image_editing_sample_o((x - 0.5) * 2,  bs_id=counter, tag=self.tag)
...
def eval_autoattack(args, config, model, x_val, y_val, adv_batch_size, log_dir, index, diffusion):
    ...
    t = torch.tensor([500]).long().to(config.device)
    x_t = noise #diffusion.q_sample(x_start, t, noise=noise)
    ...
    for it in range(1, epochs + 1):
        ...
        with torch.enable_grad():
            x_0 = model(x_t, x_start, None)  #获得逆扩散后的图片x_0
        ...
        sample = ((sample + 0) * 127.5).clamp(0, 255).to(torch.uint8)
        sample = sample.permute(0, 2, 3, 1)
        sample = sample[0].detach().cpu().numpy()
        '''
        out_dir = "images/flower/train/"
        #img = PIL.Image.fromarray(sample, 'RGB').save(out_dir + str(index) +'_' + str(it) + '.png')
        #tvu.save_image((x_0 + 1) * 0.5, os.path.join(out_dir, str(index) +'_' + str(it) + '.png'))
...
def robustness_eval(args, config, img, t, i, diffusion):
    ...
    if args.attack_version in ['standard', 'rand', 'custom']:
        eval_autoattack(args, config, model, img, y_val, adv_batch_size, log_dir, i, diffusion)
    else:
